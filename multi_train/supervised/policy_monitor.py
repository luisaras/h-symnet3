...
class PolicyMonitor(object):
	def __init__(self, envs, network, domain, verbose=False):
		self.domain = domain
		self.envs = envs
		...
		#self.graph_file = os.path.abspath(os.path.join(graph_file, "graphs"))
		self.graph_file = None

	def predict_action(self, env, state, get_attn_map, get_node_emb):
		import tensorflow as tf
		if get_attn_map:
			logits, attn = self.network.policy_prediction([state], env, training=False,
				return_attn_coef=True)
			self.attn_maps.append((state, attn.numpy()))
		elif get_node_emb:
			logits, node_emb = self.network.policy_prediction([state], env, training=False,
	...
	def simulate(self, env, policy, num_episodes=5, cache_actions=True):
		if self.verbose:
			print("env = %d" % env.get_instance_num())
		env_crewards = []
		env_lengths = []
		env_times = []
		...
		if self.graph_file is not None:
			dir_name = os.path.abspath(os.path.join(self.graph_file, str(env.get_instance_num())))
		...
		return env_crewards, env_lengths, env_times

	def simulate_all(self, policy, num_episodes=5, cache_actions=True):
		...
		for env in self.envs:
			env_crewards, env_lengths, env_times = self.simulate(env, policy, num_episodes, cache_actions)
			...
			print("Instance:", env.get_instance_num(), "Mean reward:", mean_creward)
	...
	def eval_policy(self, num_episodes=5, get_attn_map=False, get_node_emb=False, cache_actions=True):
		...
		results = self.simulate_all(policy, num_episodes, cache_actions and not get_attn_map)
