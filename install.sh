#!/usr/bin/env bash
# =============================
# Install: sudo apt install podman python3

set -e

# If an installation path is not defined, use default.
if [[ ! -v SYMNET_ROOT ]]; then
	export SYMNET_ROOT=$PWD
	echo "export SYMNET_ROOT=\"${SYMNET_ROOT}\"" >> ~/.bashrc
else
	cd ${SYMNET_ROOT}
fi
pushd ..
...
patch_nvidia() {
	if [[ "${IS_WSL}" == "true" ]]; then
		...
		# Server patch (manual driver setup)
		podman run --rm -it \
			--device /dev/nvidia0 \
